...
function my_skins_list(){
	...
	owned_skins.forEach(function(data) {
		...
		// Add Skin to List.
		$('#my_skins_list').append('<div class="io-col-20"><div class="skin-holder"><img class="skin-image" src="' + url + '"><button onclick="changeSkin(\'' + this_skin + '\')" skin_id="1" class="green small pop">Use this skin</button></div></div>');
	...
	xmlhttp.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
			...
			Object.keys(ob).forEach(function(key) {
				...
				$('#skins_list').append('<div class="io-col-20"><div class="skin-holder"><div style="background-color:' + color + '" class="skin-price">' + display_price + '</div><img class="skin-image" src="' + ob[key].skin_url + '"><button onclick="alertbuy();purchase(' + ob[key].skin_id + ', ' + price + ')" skin_id="1" skin_cost="' + price + '" style="background-color: ' + button_color + '" class="buySkin purchase">' + button_text + '</button></div></div>');
			});
			
		}
	};
		
	xmlhttp.open("GET", "/splitex.eu/assets/js/skin_list.json", true);
	xmlhttp.send();
}
...
showConfirmButton: false,
  ...
  text: 'Succesfully bought the skin!',
  icon: 'success',
  customClass: 'connected-message',
})
}
