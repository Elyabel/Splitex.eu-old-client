(function() {
    ...
    var wsUrl = null,
        ...
        KEY_TO_OPCODE = {
            " ": UINT8_CACHE[17],
            sixtyfourSplitKey: UINT8_CACHE[21],
            "q": UINT8_CACHE[18],
            "e": UINT8_CACHE[22],
            "r": UINT8_CACHE[23],
            "t": UINT8_CACHE[24],
        ...
        var sixtyfourSplitKey = localStorage.getItem("sixtyfourSplitKey");
...
Toast.fire({
  ...
  title: 'Disconnected'
        ...
        switch (packetId) {
            ...
            case 0x63: // chat message
                ...
                var server = !!(flags & 0x80),
                    bubblebalz = !!(flags & 0x100),
                    admin = !!(flags & 0x40),
                    mod = !!(flags & 0x20);

                if (server && name !== "SERVER") name = "[SERVER] " + name;
                if (bubblebalz) name = "[BubbleBALZ] " + name;
    ...
    var mouseX = NaN;
    var mouseY = NaN;
    var macroCooldown = 1 / 0;
    var macroIntervalID = null;
    var quadtree;
    ...
    var settings = {
        ...
        jellyPhysics: false,
        vrclassic: "./assets/img/vrclassic.svg"
    };
    var pressed = {
        " ": false,
        sixtyfourSplitKey: false,
        "e": false,
        "r": false,
        "t": false,
        "p": false,
    ...
    function buildGallery() {
        ...
        for (var i = 0; i < sortedKeys.length; i++) {
            ...
            c += '<button class="select-skins">Select '+ name +'</button>';
            c += '</li>';
        }
        byId("gallery-body").innerHTML = '<ul id="skinsUL">' + c + '</ul>';
        $("#actualSkin").attr("src", "./skins/" + this.skin + ".png");
    }
    ...
    function drawLeaderboard() {
        ...
        ctx.fillText("Splitex.eu", 100 - ctx.measureText("Splitex.eu").width / 2, 40);
    ...
    function drawMinimap() {
        ...
        mainCtx.strokeRect(20, 20, 150, 100);
    ...
    function cameraUpdate() {
        ...
        if (myCells.length > 0) {
            ...
            if(settings.autoZoom){
                camera.target.scale = Math.pow(Math.min(64 / s, 1), .4) && camera.userZoom; 
            }else{
            ...
            if(settings.smoothCamera){
                camera.x += (camera.target.x - camera.x) / 8;
                camera.y += (camera.target.y - camera.y) / 8;
            }else{
                camera.x += (camera.targetX - camera.x) / 2;
                camera.y += (camera.targetY - camera.y) / 2;
            }
            stats.score = score;
    ...
    Cell.prototype = {
        ...
        drawText: function(ctx) {
            if (this.s < 350 || this.jagged) return;
            var y = this.y;
            if (this.name && settings.showNames) {
                drawText(ctx, false, this.x, this.y+(this.s / 280), this.nameSize, this.drawNameSize, this.name);
    ...
    function keydown(event) {
        ...
        if (key == "enter") {
            ...
            //if (key == "w") macroIntervalID = setInterval(function() {
              //  wsSend(code);
            //}, macroCooldown);
            if (key == localStorage.getItem("minionKey")) minionControlled = !minionControlled;
        }
      switch(key) {
        case localStorage.getItem("sixtyfourSplitKey"):
        ...
        case localStorage.getItem("respawnKey"):
        isRepawn = true;
        ...
        case localStorage.getItem("feedKey"):
    ...
    function keyup(event) {
        ...
        if (key == localStorage.getItem("sixtyfourSplitKey")) {
          if (macroIntervalID != null) clearInterval(macroIntervalID);
          macroIntervalID = null;
        }
       /* if (key == localStorage.getItem("minionKey")) wsSend(UINT8_CACHE[19]);
        else if (key == ) clearInterval(macroIntervalID);*/
    }
    function handleScroll(event) {
        if (event.target !== mainCanvas) return;
        camera.userZoom *= event.deltaY > 0 ? 0.8 : 1.2;
        camera.userZoom = Math.max(camera.userZoom, settings.moreZoom ? 0 : 1);
